...
//#define FOR_BOOST 1
//目前boost的支持仅限于返回share_ptr对象到lua，包括vector，list，set，map
...
typedef zxero::int32	ScriptID_t;			//脚本
...
{
public:
    explicit lua_exception(const char* err_):
		m_err(err_)
	{}
    explicit lua_exception(const string& err_):
		m_err(err_)
	{
	}
    ~lua_exception() throw (){}
...
//#if FOR_BOOST
template<typename T>
struct basetype_ptr_traits_t<boost::shared_ptr<T>>
{
	typedef boost::shared_ptr<T> arg_type_t;
};
template<typename T>
struct basetype_ptr_traits_t<boost::shared_ptr<T> &>
{
...
template<typename T>
struct basetype_ptr_traits_t<const boost::shared_ptr<T> &>
{
	typedef boost::shared_ptr<T> arg_type_t;
};
template<typename T>
...
template<> struct lua_op_t<int64_t>
{
	static void push_stack(lua_State* ls_, int64_t arg_)
	{
		lua_pushinteger(ls_, (lua_Integer)arg_);
	}
	...
	static int get_ret_value(lua_State* ls_, int pos_, int64_t& param_)
	{
		...
		param_ = (int64_t)lua_tointeger(ls_, pos_);
		return 0;
	}

	static int lua_to_value(lua_State* ls_, int pos_, int64_t& param_)
	{
		param_ = (int64_t)luaL_checkinteger(ls_, pos_);
		return 0;
	}
};
...
template<> struct lua_op_t<uint64_t>
{
	...
	static int get_ret_value(lua_State* ls_, int pos_, uint64_t& param_)
	{
		...
		param_ = (uint64_t)lua_tointeger(ls_, pos_);
		return 0;
	}

	static int lua_to_value(lua_State* ls_, int pos_, uint64_t& param_)
	{
		param_ = (uint64_t)luaL_checkinteger(ls_, pos_);
		return 0;
	}
};
...
template<>
struct lua_op_t<google::protobuf::Message*>
{
	static void push_stack(lua_State* ls_, google::protobuf::Message* arg_)
	{
		if (arg_ == nullptr) {
			lua_pushnil(ls_);
			return;
		}
		void* ptr = lua_newuserdata(ls_, sizeof(userdata_for_object_t<google::protobuf::Message>));
		new (ptr) userdata_for_object_t<google::protobuf::Message>(arg_);
		std::string name = "fflua." + arg_->GetDescriptor()->name();
		luaL_getmetatable(ls_, name.c_str());
		lua_setmetatable(ls_, -2);
	}
	static int lua_to_value(lua_State* ls_, int pos_, google::protobuf::Message*& param_)
	{
		void *arg_data = lua_touserdata(ls_, pos_);
		...
		google::protobuf::Message* ret_ptr = ((userdata_for_object_t<google::protobuf::Message>*)arg_data)->obj;
		if (NULL == ret_ptr)
		{
			char buff[128];
			SPRINTF_F(buff, sizeof(buff), "`%s` object ptr can't be null",
				lua_type_info_t<google::protobuf::Message>::get_name());
			luaL_argerror(ls_, pos_, buff);
		}
		std::string name = "fflua." + ret_ptr->GetDescriptor()->name();
		luaL_getmetatable(ls_, name.c_str());
		if (0 == lua_rawequal(ls_, -1, -2))
		{
			lua_getfield(ls_, -2, INHERIT_TABLE);
...
//#if FOR_BOOST
template<typename T>
struct lua_op_t<boost::shared_ptr<T>>
{
	static void push_stack(lua_State* ls_, boost::shared_ptr<T>& arg_)
	{
		if (arg_ == nullptr) {
			lua_pushnil(ls_);
			return;
		...
		void* ptr = lua_newuserdata(ls_, sizeof(userdata_for_object_t<T>));
		new (ptr) userdata_for_object_t<T>(arg_.get());

		luaL_getmetatable(ls_, lua_type_info_t<T>::get_name());
		lua_setmetatable(ls_, -2);
	}
	static void push_stack(lua_State* ls_, const boost::shared_ptr<T>& arg_)
	{
		if (arg_ == nullptr) {
			lua_pushnil(ls_);
			return;
		...
		void* ptr = lua_newuserdata(ls_, sizeof(userdata_for_object_t<const T>));
		new (ptr) userdata_for_object_t<const T>(arg_.get());
...
//#if FOR_BOOST
template<typename T>
struct lua_op_t<vector<boost::shared_ptr<T>> >
{
	static void push_stack(lua_State* ls_, const vector<boost::shared_ptr<T>>& arg_)
	{
		lua_newtable(ls_);
		typename vector<boost::shared_ptr<T>>::const_iterator it = arg_.begin();
		for (int i = 1; it != arg_.end(); ++it, ++i)
		{
			lua_op_t<int>::push_stack(ls_, i);
			lua_op_t<T*>::push_stack(ls_, *it->get());
			lua_settable(ls_, -3);
		}
	}
...
//#if FOR_BOOST
template<typename T>
struct lua_op_t<list<boost::shared_ptr<T>> >
{
	static void push_stack(lua_State* ls_, const list<boost::shared_ptr<T>>& arg_)
	{
		lua_newtable(ls_);
		typename list<boost::shared_ptr<T>>::const_iterator it = arg_.begin();
		for (int i = 1; it != arg_.end(); ++it, ++i)
		{
			lua_op_t<int>::push_stack(ls_, i);
			lua_op_t<T*>::push_stack(ls_, *it->get());
...
//#if FOR_BOOST
template<typename T>
struct lua_op_t<set<boost::shared_ptr<T>> >
{
	static void push_stack(lua_State* ls_, const set<boost::shared_ptr<T>>& arg_)
	{
		lua_newtable(ls_);
		typename set<T>::const_iterator it = arg_.begin();
		for (int i = 1; it != arg_.end(); ++it, ++i)
...
//#if FOR_BOOST
template<typename K, typename V>
struct lua_op_t<map<K, boost::shared_ptr<V>> >
{
	static void push_stack(lua_State* ls_, const map<K, boost::shared_ptr<V>>& arg_)
	{
		lua_newtable(ls_);
		typename map<K, boost::shared_ptr<V>>::const_iterator it = arg_.begin();
		for (; it != arg_.end(); ++it)
		{
			lua_op_t<K>::push_stack(ls_, it->first);
			lua_op_t<V*>::push_stack(ls_, it->second.get());
			lua_settable(ls_, -3);
		}
	}
};
