...
function x001543_LuaCreateItem(player,item_data,config)
	return ResultCode_UnknownError;
end

--添加到背包时候用，create之前，实际上这种道具没有创建实体，只是用来触发一段逻辑
function x001543_LuaAutoUse(player,config,count)
	if config:id() ~=20125036 then	
		return ResultCode_UnknownError;
	end
	...
	local tb_item = LuaMtDrawManager():GetDrawList(DrawGroup_CrystalFree,player:PlayerLevel())
...
function x001543_LuaUseItem(player,item_data,config,target)
	return ResultCode_UnknownError;
end
