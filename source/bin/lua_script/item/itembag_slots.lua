--修复石
function x001520_LuaCreateItem(player,item_data,config)
	return ResultCode_ResultOK
end

--添加到背包时候用，create之前，实际上这种道具没有创建实体，只是用来触发一段逻辑
function x001520_LuaAutoUse(player,config,count)
	return ResultCode_UnknownError
end

function x001520_LuaUseItem(player,item_data,config,target)	
	local id = config:id()
	if id ~= 20140001 then
