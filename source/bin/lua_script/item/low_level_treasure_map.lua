...
function x001503_GenTrigger(player)
	local allweight = 0;
...
function x001503_GenGold(player)
	local allweight = 0;
...
function x001503_GenItem(player)
	local allweight = 0;
...
function x001503_GenSkyFallNpc(player)
	...
	if size >0 then
		...
		--刷一半就行
		local count = binggo_group[4]/2;
		local pos_node_list = LuaMtMonsterManager():GenRandomPos(binggo_group[2], count);
		...
		--系统公告	
		local scene = LuaSceneManager():GetSceneById(binggo_group[1])
		local scene_config = scene:Config()				
		Lua_send_run_massage("treasure_map_notify_06|"..player:Name().."|"..scene_config:scene_name()); --AAAA在挖掘遗迹时，不小心惊动了天灾军团，天灾军团出现在BBBB
...
function x001503_GenMonsterGroup(player)
...
--------------------------------------		
function x001503_LuaCreateItem(player,item_data,config)
...
function x001503_LuaAutoUse(player,config,count)
	return ResultCode_UnknownError;
end

function x001503_LuaUseItem(player,item_data,config,target)
	if config:id()==20125001 then	
		--位置校验
		--print("item_data:param()"..item_data:param())
		local treasure_pos = player:GetParams32("treasure_pos")
		if player:ValidPosition(treasure_pos,2500)==false then
			player:SendCommonResult(ResultOption_Item_Common,ResultCode_InvalidPosition)
		...
		local trigger = x001503_GenTrigger(player)
		--local trigger = "skyfall"--GenTrigger(player);
		--print("trigger"..trigger)
		if trigger == "gold" then
			local count = x001503_GenGold(player)
			player:AddItemByIdWithNotify(TokenType_Token_Gold,count,ItemAddLogType_AddType_UseItem,20125001,1)
		elseif trigger == "item" then
			local itemid, count = x001503_GenItem(player)
			player:AddItemByIdWithNotify(itemid,count,ItemAddLogType_AddType_UseItem,20125001,1)
		elseif trigger == "skyfall" then
			if x001503_GenSkyFallNpc(player) then
				player:SendClientNotify("treasure_map_notify_01",-1,-1); --你放出了天灾军团
			end
		elseif trigger == "monster" then
			local monster_group = x001503_GenMonsterGroup(player)			
