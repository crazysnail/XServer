...
-----------------------------------------------for server---------------------------------------------
function x002629_OnUpdateAcrConfig(params)
	config = acr_static_config[2629]
	return ResultCode_ResultOK
end

function x002629_IsAcrActive(player)
...
function x002629_OnResolve(player)
end

-----------------------------------------------
function x002629_OnRegisterFunction(proxy)
	
	if proxy ~= nil then
	end
end

function x002629_DoReward(player,params)
	...
	local flag_index = g_acr_datacell_index.czlb_1_flag
	...
	--扣符石
	if player:DelItemById(TokenType_Token_Crystal,group[1][1],ItemDelLogType_DelType_Acr,2629) == false then
	...
	for i=1,items,1 do
		...
		player:AddItemByIdWithNotify(id,count,ItemAddLogType_AddType_Acr,2629,1)
	end
	
	return ResultCode_ResultOK
end

function x002629_xOnNewDay(player,params)
	return x002629_OnResetData(player,params)
end


function x002629_OnResetData(player,params)
	...
	dc_container:set_data_32( CellLogicType_AcRewardCell, g_acr_datacell_index.czlb_1_flag, 0, true)
