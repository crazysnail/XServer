...
-----------------------------------------------for server---------------------------------------------
function x002626_OnUpdateAcrConfig(params)
	config = acr_static_config[2626]
	return ResultCode_ResultOK
end

function x002626_IsAcrActive(player)
	--时间
...
-----------------------------------------------
function x002626_OnRegisterFunction(proxy)
	
	if proxy ~= nil then
		proxy:RegFunc("xOnNewWeek"			,2626)	
	end
end

function x002626_OnResolve(player,params)
	--做个开启保护
	if x002626_IsAcrActive() ~= ResultCode_ResultOK then
	...
	--print("index ".. index .. " effect_value "..effect_value )
	local group ={}
	...
	local reward_flag =dc_container:get_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.zlb_reward_flag)	
	local new_flag = dc_container:set_bit_data_32(reward_flag,offset)
	dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.zlb_reward_flag,new_flag,true)	
	
	return 
end


function x002626_DoReward(player,params)
	...
	if times<group[1][2] then
		...
		dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.zlb_reward_flag,new_flag_data,true)
	else
	...
	for i=1,items,1 do
		...
		player:AddItemByIdWithNotify(id,count,ItemAddLogType_AddType_Acr,2626,1)
	end
...
function x002626_xOnNewWeek(player,params)
	return x002626_OnResetData(player,params)
end

function x002626_OnResetData(player,params)
	...
	dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.zlb_reward_flag,0,true)
