...
config.script_id		=	002605		--活动id
config.type_name		=	"等级冲刺"	--活动名字
...
-----------------------------------------------for server---------------------------------------------
function x002605_OnUpdateAcrConfig(params)
	return ResultCode_ResultOK
end

function x002605_IsAcrActive(player)
...
function x002605_OnResolve(player)	
	...
	if x002605_IsAcrActive(player) == ResultCode_ResultOK then
		config_check = 1
	end
	if config_check == 1 then	
		...
		for index=0,count-1,1 do 
			local flag_data=dc_container:get_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.level_race_flag)
			if dc_container:check_bit_data_32(flag_data,index) == false then		
				----print("flag_data"..flag_data)
				local reward_level = config.items[index+1][1]	
				local now_level = player:PlayerLevel();
				----print("index"..index)
				if reward_level <= now_level then
					local new_data = dc_container:set_bit_data_32(flag_data,index)
					dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.level_race_flag,new_data,true)
				end
			end
		end
	end
...
function x002605_OnUpdate(player)	
...
function x002605_DoReward(player,params)
	...
	dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.level_race_flag,new_flag_data,true)
	...
	for i=2,items,1 do
		...
		player:AddItemByIdWithNotify(id,count,ItemAddLogType_AddType_Acr,2605,1)
	end
...
----------------------------------------------------------------------------
function x002605_OnRegisterFunction(proxy)
	
	if proxy ~= nil then
		proxy:RegFunc("xOnLevelUpTo"		,2605)	
	end
end

function x002605_xOnLevelUpTo(player,params)
	x002605_OnResolve(player)	
	return ResultCode_ResultOK
end
function x002605_xOnNewDay(player,params)
