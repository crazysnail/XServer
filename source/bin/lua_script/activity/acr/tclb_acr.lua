...
-----------------------------------------------for server---------------------------------------------
function x002625_OnUpdateAcrConfig(params)
	config = acr_static_config[2625]
	return ResultCode_ResultOK
end
function x002625_IsAcrActive(player)
...
-----------------------------------------------
function x002625_OnRegisterFunction(proxy)
	
	if proxy ~= nil then
	end
end

function x002625_OnResolve(player,params)
		--做个开启保护
	if x002625_IsAcrActive() ~= ResultCode_ResultOK then
	...
	local reward_flag =dc_container:get_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.tclb_reward_flag)	
	local new_flag = dc_container:set_bit_data_32(reward_flag,offset)
	dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.tclb_reward_flag,new_flag,true)	
	
	return ResultCode_ResultOK
end


function x002625_DoReward(player,params)
	...
	if times<group[1][2] then
		...
		dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.tclb_reward_flag,new_flag_data,true)
	else
	...
	for i=1,items,1 do
		...
		player:AddItemByIdWithNotify(id,count,ItemAddLogType_AddType_Acr,2625,1)
	end
	
	return ResultCode_ResultOK
end

function x002625_OnResetData(player,params)
	...
	dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.tclb_reward_flag,0,true)
