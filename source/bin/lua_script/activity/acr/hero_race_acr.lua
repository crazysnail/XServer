...
config.script_id		=	002607		--活动id
config.type_name		=	"英雄收集"	--活动名字
...
-----------------------------------------------for server---------------------------------------------
function x002607_OnUpdateAcrConfig(params)
	return ResultCode_ResultOK
end

function x002607_IsAcrActive(player)
...
function x002607_OnResolve(player)
	local dc_container = player:DataCellContainer()
	if dc_container == nil then		
		return
	end
	local config_check= 0
	if x002607_IsAcrActive(player) == ResultCode_ResultOK then
		config_check = 1
	end
	if config_check == 1 then	
		local count = #config.items
		for index=0,count-1,1 do 
			local flag_data=dc_container:get_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.hero_race_flag)
			if dc_container:check_bit_data_32(flag_data,index) == false then		
				local reward_count = config.items[index+1][1]	
				local actor_count = #player:Actors()-1;--主角不算
				if reward_count <= actor_count then
					local new_day = dc_container:set_bit_data_32(flag_data,index)
					dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.hero_race_flag,new_day,true)
				end
			end
		end
	end
...
function x002607_OnUpdate(player)
	local config_check= 0
	if x002607_IsAcrActive(player) == ResultCode_ResultOK then
		config_check = 1
	end
...
function x002607_DoReward(player,params)
	...
	dc_container:set_data_32(CellLogicType_AcRewardCell, g_acr_datacell_index.hero_race_flag,new_flag_data,true)
	...
	for i=2,items,1 do
		...
		player:AddItemByIdWithNotify(id,count,ItemAddLogType_AddType_Acr,2607,1)
	end
...
----------------------------------------------------------------------------
function x002607_OnRegisterFunction(proxy)
	
	if proxy ~= nil then
		proxy:RegFunc("xAddHero"			,2607)	
	end
end

function x002607_xAddHero(player,params)
	x002607_OnResolve(player)
	return ResultCode_ResultOK
end
function x002607_xOnNewDay(player,params)
