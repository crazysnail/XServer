...
-----------------------------------------------for server---------------------------------------------
function x002642_OnUpdateAcrConfig(params)
	config = acr_static_config[2642]
	return ResultCode_ResultOK
end

function x002642_IsAcrActive(player)
...
function x002642_OnResolve(player)
end

-----------------------------------------------
function x002642_OnRegisterFunction(proxy)
	
	if proxy ~= nil then
	end
end

function x002642_DoReward(player,params)
	...
	local flag_index = g_acr_datacell_index.czlb_5_flag
	...
	--扣符石
	if player:DelItemById(TokenType_Token_Crystal,group[1][1],ItemDelLogType_DelType_Acr,2642) == false then
	...
	for i=1,items,1 do
		...
		player:AddItemByIdWithNotify(id,count,ItemAddLogType_AddType_Acr,2642,1)
	end
	
	return ResultCode_ResultOK
end

function x002642_xOnNewDay(player,params)
	return x002642_OnResetData(player,params)
end


function x002642_OnResetData(player,params)
	...
	dc_container:set_data_32( CellLogicType_AcRewardCell, g_acr_datacell_index.czlb_5_flag, 0, true)
