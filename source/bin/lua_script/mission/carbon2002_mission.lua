...
local mission_type = MissionType_Carbon2002Mission
local activity_type=ActivityType.TeamCarbon2002

local process = MissionCellIndex.Carbon2002_Process
local last_id = MissionCellIndex.Carbon2002_Last
local begin_mission = 67101

---------------------------------------------------------
function x000522_xOnDrop(player, params)	
	--放弃归0
...
function x000522_xOnFinishEnd(player, params)	
...
function x000522_xOnReset(player, params)	
...
function x000522_xOnAccept(player,params)
	if params["type"] == mission_type then
		...
		local next_mission = x000522_GetNextMission(player)
...
function x000522_xOnFinishBegin(player,params)
...
--------------------------------------------------
function x000522_GetMissionParams(player)
	 return {last_id,process}
end
function x000522_GetNextMission(player)
...
function x000522_OnAddBegin(player,config)
...
function x000522_OnClone(player, target, config)
...
function x000522_OnAddEnd(player,config)
