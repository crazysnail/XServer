...
local activity_type=ActivityType.BigThreeCarbon
local mission_type = MissionType_BigThreeCarbonMission

local process = {MissionCellIndex.CarbonBigThree_process1,MissionCellIndex.CarbonBigThree_process2,MissionCellIndex.CarbonBigThree_process3}
local last_id = MissionCellIndex.CarbonBigThree_SelfLast
--一周每天任务组
local mission_group={
{2 ,4 ,7},
{6 ,8 ,9},
...
---------------------------------------------------------
function x000514_xOnDrop(player, params)	
...
function x000514_xOnReset(player, params)
...
function x000514_xOnAccept(player,params)
	if params["type"] == mission_type then	
		...
		local next_mission = x000514_GetNextMission(player)
...
function x000514_xOnFinishEnd(player,params)
	...
	if last_mission_id == config:id() then
		...
		--重置一下lastid，保证能重入
		local index = x000514_GetIndex(player,config)	
...
function x000514_xOnFinishBegin(player,params)
	...
	if cell_container:check_bit_data_32(process_flags,circle) == false then
		...
		--取得波数
		local ids = LuaMtMissionManager():GetCarbonBigThreeMission(config:group())
...
------------------------------------------------------------------
function x000514_GetMissionParams(player)
	local index = 1
	local cell_container = player:DataCellContainer()	
	...
	if last > 0 then		
		...
		if config ~= nil then
			index = x000514_GetIndex(player,config)
		end
	end
	return {last_id,process[index]}
end
function x000514_GetNextMission(player)
	...
	local next_mission = LuaMtMissionManager():GenCarbonBigThreeMission(lastgroup,lastmission)
	...
	elseif next_mission == -2 then
		...
		lastmission = -1
		next_mission = LuaMtMissionManager():GenCarbonBigThreeMission(lastgroup,lastmission)
		--print("next_mission"..next_mission)
		--是真没任务了
		if next_mission == -1 then
			return -1
...
function x000514_OnAddBegin(player,config)
...
function x000514_OnClone(player, target, config)
...
function x000514_OnAddEnd(player,config)
...
function x000514_GetIndex(player,config)
...
function x000514_ResolveAllReward(player)
