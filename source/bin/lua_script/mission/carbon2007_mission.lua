...
local mission_type = MissionType_Carbon2007Mission
local activity_type=ActivityType.TeamCarbon2007

local process = MissionCellIndex.Carbon2007_Process
local last_id = MissionCellIndex.Carbon2007_Last
local begin_mission = -1

---------------------------------------------------------
function x000527_xOnDrop(player, params)	
	--放弃归0
...
function x000527_xOnFinishEnd(player, params)	
...
function x000527_xOnReset(player, params)	
...
function x000527_xOnAccept(player,params)
	if params["type"] == mission_type then
		...
		local next_mission = x000527_GetNextMission(player)
...
function x000527_xOnFinishBegin(player,params)
...
--------------------------------------------------
function x000527_GetMissionParams(player)
	 return {last_id,process}
end
function x000527_GetNextMission(player)
...
function x000527_OnAddBegin(player,config)
...
function x000527_OnClone(player, target, config)
...
function x000527_OnAddEnd(player,config)
