...
local mission_type = MissionType_Carbon2004Mission
local activity_type=ActivityType.TeamCarbon2004

local process = MissionCellIndex.Carbon2004_Process
local last_id = MissionCellIndex.Carbon2004_Last
local begin_mission = 67301

---------------------------------------------------------
function x000524_xOnDrop(player, params)	
	--放弃归0
...
function x000524_xOnFinishEnd(player, params)		
...
function x000524_xOnReset(player, params)	
...
function x000524_xOnAccept(player,params)
	if params["type"] == mission_type then
		...
		local next_mission = x000524_GetNextMission(player)
...
function x000524_xOnFinishBegin(player,params)
...
--------------------------------------------------
function x000524_GetMissionParams(player)
	 return {last_id,process}
end
function x000524_GetNextMission(player)
...
function x000524_OnAddBegin(player,config)
...
function x000524_OnClone(player, target, config)
...
function x000524_OnAddEnd(player,config)
