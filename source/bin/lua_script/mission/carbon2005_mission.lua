...
local mission_type = MissionType_Carbon2005Mission
local activity_type=ActivityType.TeamCarbon2005

local process = MissionCellIndex.Carbon2005_Process
local last_id = MissionCellIndex.Carbon2005_Last
local begin_mission = 67201

---------------------------------------------------------
function x000525_xOnDrop(player, params)	
	--放弃归0
...
function x000525_xOnFinishEnd(player, params)	
...
function x000525_xOnReset(player, params)	
...
function x000525_xOnAccept(player,params)
	if params["type"] == mission_type then
		...
		local next_mission = x000525_GetNextMission(player)
...
function x000525_xOnFinishBegin(player,params)
...
--------------------------------------------------
function x000525_GetMissionParams(player)
	 return {last_id,process}
end
function x000525_GetNextMission(player)
...
function x000525_OnAddBegin(player,config)
...
function x000525_OnClone(player, target, config)
...
function x000525_OnAddEnd(player,config)
