...
local mission_type = MissionType_Carbon2001Mission
local activity_type=ActivityType.TeamCarbon2001

local process = MissionCellIndex.Carbon2001_Process
local last_id = MissionCellIndex.Carbon2001_Last
local begin_mission = 67001

---------------------------------------------------------
function x000521_xOnDrop(player, params)	
	--放弃归0
...
function x000521_xOnFinishEnd(player, params)		
...
function x000521_xOnReset(player, params)	
...
function x000521_xOnAccept(player,params)
	if params["type"] == mission_type then
		...
		local next_mission = x000521_GetNextMission(player)
...
function x000521_xOnFinishBegin(player,params)
...
--------------------------------------------------
function x000521_GetMissionParams(player)
	 return {last_id,process}
end
function x000521_GetNextMission(player)
...
function x000521_OnAddBegin(player,config)
...
function x000521_OnClone(player, target, config)
...
function x000521_OnAddEnd(player,config)
